...
$this->Html->script('Croogo.jquery/thickbox-compressed', ['block' => 'scriptBottom']);
$this->Html->css('Croogo.thickbox', ['inline' => false]);
...
<?php
$this->append('actions');
echo $this->Croogo->adminAction(__d('croogo', 'New Attachment'),
    array_merge(['controller' => 'assets_attachments', 'action' => 'add', 'editor' => 1],
        ['?' => $this->request->query]));
...
$filters .= $this->Form->input('filter', [
    'label' => false,
    'placeholder' => true,
    'div' => 'input text span4',
]);
$filters .= $this->Form->input('filename', [
    'label' => false,
    'placeholder' => true,
    'div' => 'input text span4',
]);
$filters .= $this->Form->submit(__d('croogo', 'Filter'), [
...
foreach ($attachments as $attachment):
    ...
    $resizeUrl = array_merge(['action' => 'resize', $attachment['AssetsAttachment']['id'], 'ext' => 'json'],
        ['?' => $query]);

    if (!isset($this->request->query['all']) && !isset($this->request->query['asset_id'])) {
        $actions[] = $this->Croogo->adminRowAction('', $deleteUrl, [
            'icon' => $_icons['delete'],
            'tooltip' => __d('croogo', 'Delete Attachment'),
        ], __d('croogo', 'Are you sure?'));
    } elseif (isset($this->request->query['manage']) && isset($this->request->query['asset_id'])) {
        $actions[] = $this->Croogo->adminRowAction('', $deleteAssetUrl, [
            'icon' => $_icons['delete'],
            'tooltip' => __d('croogo', 'Delete Attachment version'),
        ], __d('croogo', 'Are you sure?'));
    }

    if ($mimeType === 'image' && $attachment['AssetsAttachment']['hash'] == $attachment['AssetsAsset']['hash']) {
        $resizeUrl = array_merge(['action' => 'resize', $attachment['AssetsAttachment']['id'], 'ext' => 'json'],
            ['?' => $query]);
    ...
    elseif ($mimeType === 'image'):
        ...
        $actions[] = $this->Html->link('', $detailUrl, [
            'icon' => 'suitcase',
    ...
    if ($mimeType == 'image') {
        $img = $this->AssetsImage->resize($attachment['AssetsAsset']['path'], 100, 200,
            ['adapter' => $attachment['AssetsAsset']['adapter']]);
        $thumbnail = $this->Html->link($img, $attachment['AssetsAsset']['path'], [
                'class' => 'thickbox',
                'escape' => false,
        ...
        if (!empty($attachment['AssetsAssetUsage']['type']) &&
            ...
            $thumbnail .= $this->Html->div(null,
                $this->Html->link($this->Html->tag('span', $attachment['AssetsAssetUsage']['type'],
        ...
        $thumbnail = $this->Html->image('/croogo/img/icons/page_white.png') .
            ' ' .
            $attachment['AssetsAsset']['mime_type'] .
            ' (' .
            $this->Filemanager->filename2ext($attachment['AssetsAttachment']['slug']) .
            ')';
    ...
    $url = $this->Html->link(Router::url($attachment['AssetsAsset']['path']), $attachment['AssetsAsset']['path'], [
    ...
    $title .= $this->Html->para(null,
        $this->Text->truncate($attachment['AssetsAsset']['filename'], 30) . '&nbsp;' . $urlPopover,
        ['title' => $attachment['AssetsAsset']['filename']]);

    $title .= $this->Html->para(null,
        'Dimension: ' . $attachment['AssetsAsset']['width'] . ' x ' . $attachment['AssetsAsset']['height']);

    $title .= $this->Html->para(null, 'Size: ' . $this->Number->toReadableSize($attachment['AssetsAsset']['filesize']));

    if (empty($this->request->query['all']) && empty($this->request->query['asset_id'])) {
        $title .= $this->Html->para(null, 'Number of versions: ' . $attachment['AssetsAttachment']['asset_count']);
    }
